...
const OptionWheel = forwardRef<OptionWheelHandle, OptionWheelProps>(({
  ...
  const runFrame = useCallback((now: number) => {
    ...
    if (slots.length !== poolCount) {
      ...
      slots = Array.from({ length: poolCount }, (_, k) => base - poolHalf + k);
    ...
    for (let i = 0; i < poolCount; i++) {
      ...
      for (let s = 1; s <= minorSteps; s++) {
        ...
        if (!minorEl) continue;
        const m = slots[idx] + ((slots[idxNext] - slots[idx]) * s) / (minorSteps + 1);
        ...
        Array.from(
          ...
          (_, index) => (
            <div
              ...
              style={{ width: `${minorNotchLength}px`, height: `${minorNotchThickness}px` }}
            />
          )
        )}
